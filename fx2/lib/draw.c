...
static	int							stride;
static	int							bpp = 8;
static	struct fb_cmap				cmap,oldcmap;
...
// 	else
	{
		fbdevice = open( fbdevname, O_RDWR );
		if ( fbdevice == -1 )
		{
	...
	if (ioctl(fbdevice, FBIOGET_VSCREENINFO, &screeninfo)<0)
	{
		perror("failed - FBIOGET_VSCREENINFO");
		return(-1);
	}
	if (ioctl(fbdevice, FBIOGETCMAP, &cmap )<0)
		perror("FBGetCMap");

	memcpy(&oldscreen,&screeninfo,sizeof(screeninfo));
	memcpy(&oldcmap,&cmap,sizeof(cmap));
...
// 	if (ioctl(fbdevice, FBIOPUTCMAP, &cmap )<0)
// 		perror("FBSetCMap");
	...
	if (ioctl(fbdevice, FBIOGET_FSCREENINFO, &fix)<0)
	{
	...
	lfb=(unsigned char*)mmap(0,available,PROT_WRITE|PROT_READ, MAP_SHARED, fbdevice, 0);
...
void	FBClose( void )
{
	...
	if (available)
	{
// 		if (ioctl(fbdevice, FBIOPUTCMAP, &oldcmap )<0)
// 			perror("FBSetOldCMap");

		if (ioctl(fbdevice, FBIOPUT_VSCREENINFO, &oldscreen)<0)
			perror("FBSetOldMode");
