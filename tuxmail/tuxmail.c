...
FT_Error MyFaceRequester(FTC_FaceID face_id, FT_Library library, FT_Pointer request_data, FT_Face *aface)
{
	...
	{
		printf ( "TuxMail <open Font \"%s\" failed>\n", ( char* ) face_id );
	}
		...
		for ( row = 0; row < sbit->height; row++ )
		{
			for ( pitch = 0; pitch < sbit->pitch; pitch++ )
			{
				for ( bit = 7; bit >= 0; bit-- )
				{
					...
					{
						memcpy ( lbb + startx*4 + sx*4 + ( sbit->left + kerning.x + x ) *4 + fix_screeninfo.line_length* ( starty + sy - sbit->top + y ),bgra[pal][color],4 );
					}
...
//			memset(lbb + startx + sx + var_screeninfo.xres*(starty + sy), color, ex-sx + 1);
			...
			{
				memcpy ( lbb + startx*4 + sx*4 + ( tx*4 ) + fix_screeninfo.line_length* ( starty + sy ),bgra[pal][color],4 );
			}
		}
	}
	else
	{
		...
		{
			memcpy ( lbb + startx*4+loop*4 + fix_screeninfo.line_length* ( sy+starty ), bgra[pal][color], 4 );
			memcpy ( lbb + startx*4+loop*4 + fix_screeninfo.line_length* ( sy+1+starty ), bgra[pal][color], 4 );
			memcpy ( lbb + startx*4+loop*4 + fix_screeninfo.line_length* ( ey-1+starty ), bgra[pal][color], 4 );
			memcpy ( lbb + startx*4+loop*4 + fix_screeninfo.line_length* ( ey+starty ), bgra[pal][color], 4 );
		}
		...
		{
			memcpy ( lbb + startx*4+sx*4 + fix_screeninfo.line_length* ( loop+starty ), bgra[pal][color], 4 );
			memcpy ( lbb + startx*4+ ( sx+1 ) *4 + fix_screeninfo.line_length* ( loop+starty ), bgra[pal][color], 4 );
			memcpy ( lbb + startx*4+ ( ex-1 ) *4 + fix_screeninfo.line_length* ( loop+starty ), bgra[pal][color], 4 );
			memcpy ( lbb + startx*4+ex*4 + fix_screeninfo.line_length* ( loop+starty ), bgra[pal][color], 4 );
		}
	}
}
...
void RenderArrow ( int sx, int sy, int trend, int color )
{
	int x, y;
	...
	for ( y = 0; y < 14; y++ )
	{
		for ( x = 0; x < 14; x++ )
		{
			if ((trend == UP && scroll_up[x + y*14]) || (trend == DOWN && scroll_dn[x + y*14]))
			{
				memcpy ( lbb + (startx + sx + x)*4 + fix_screeninfo.line_length* ( starty + sy + y ), bgra[pal][color], 4 );
			}
		}
	}
}
...
{
	int maxw = MBOX_WIDTH_NORMAL - 4;

	//Layout PaintMessagebox
	PaintMessageBox (MBOX_WIDTH_NORMAL, MBOX_HEIGHT_NORMAL, BUT_OKEXIT);

	RenderString ( header, mx-bw+2, my-bh+35, maxw, CENTER, BIG, ORANGE );
	RenderString ( question, mx-bw+2, my-bh+87, maxw, CENTER, BIG, WHITE );
...
void ShowMailHeader ( char* szAction )
{
	...
	if ( p )
	{
		*p = 0;
		...
		RenderString ( szAction, 2*BORDERSIZE+120, BORDERSIZE+FONTHEIGHT_SMALL-2  , viewx-4*BORDERSIZE-120, LEFT, SMALL, WHITE );
		szAction = ++p;
	}
	if ( p )
	{
	...
	if ( p )
	{
		*p = 0;
		...
		RenderString ( szAction, 2*BORDERSIZE+120, BORDERSIZE+2*FONTHEIGHT_SMALL-2  , viewx-4*BORDERSIZE-120, LEFT, SMALL, WHITE );
		szAction = ++p;
	}
	if ( p )
	{
	...
	if ( p )
	{
		*p = 0;
		...
		RenderString ( szAction, 2*BORDERSIZE+120, BORDERSIZE+3*FONTHEIGHT_SMALL-2 , viewx-4*BORDERSIZE-120, LEFT, SMALL, WHITE );
		szAction = ++p;
	}
	if ( p )
	{
	...
	if ( p )
	{
		*p = 0;
		...
		RenderString ( szAction, 2*BORDERSIZE+520, BORDERSIZE+3*FONTHEIGHT_SMALL-2 , viewx-4*BORDERSIZE-520, LEFT, SMALL, WHITE );
		szAction = ++p;
	}
...
void ShowMailFile ( char* filename, char* szAction )
{
	...
	while ( 1 )
	{
		...
		if ( iRetfgets )
		{
			...
			RenderString ( line, 2*BORDERSIZE+2, 4*BORDERSIZE+3*FONTHEIGHT_SMALL+2+row*fHeight -FONT_OFFSET, viewx-4*BORDERSIZE, fAssign, fSize, WHITE );
		}
...
void PaintSmtpMailHeader ( int nEditDirectStyle , int nConfigPage )
{
	...
	RenderBox ( 3*( KEYBOX_WIDTH+KEYBOX_SPACE )+14, VIEWY-INFOBOXY, 3*( KEYBOX_WIDTH+KEYBOX_SPACE )+16,VIEWY, GRID, SKIN2 );
	...
	else
	{
		...
		if ( nConfigPage < 10 )
		{
			RenderString ( ( osd == 'G' ) ? "POP3 / IMAP / SMTP Einstellungen Mailbox:" : "POP3 / IMAP / SMTP setting mailbox:", 2*BORDERSIZE, BORDERSIZE+3*FONTHEIGHT_SMALL-6  , viewx-74-2*BORDERSIZE, LEFT, NORMAL, ORANGE );
			sprintf ( linebuffer,"%d",nConfigPage );
			RenderString ( linebuffer, 2*BORDERSIZE+520, BORDERSIZE+3*FONTHEIGHT_SMALL-6  , viewx-74-2*BORDERSIZE, LEFT, NORMAL, WHITE );
			...
			for ( i=2; i<15; i++ )
			{
				...
				RenderString ( linebuffer, 2*BORDERSIZE, 4*BORDERSIZE+ ( 2+i ) *FONTHEIGHT_SMALL  , viewx-4*BORDERSIZE, LEFT, SMALL, ORANGE );
				RenderString ( ":", 2*BORDERSIZE+75, 4*BORDERSIZE+ ( 2+i ) *FONTHEIGHT_SMALL  , 10, LEFT, SMALL, ORANGE );
			}
		...
		else
		{
			...
			if ( nConfigPage == 10 )
			{
				...
				for ( i=2; i<14; i++ )
				{
					...
					RenderString ( linebuffer, 2*BORDERSIZE, 4*BORDERSIZE+ ( 2+i ) *FONTHEIGHT_SMALL  , viewx-4*BORDERSIZE, LEFT, SMALL, ORANGE );
					if (i < 13)
						RenderString ( ":", 2*BORDERSIZE+143, 4*BORDERSIZE+ ( 2+i ) *FONTHEIGHT_SMALL  , 10, LEFT, SMALL, ORANGE );
				}
			}
			else
			{
				...
				for ( i=2; i<14; i++ )
				{
					...
					RenderString ( linebuffer, 2*BORDERSIZE, 4*BORDERSIZE+ ( 2+i ) *FONTHEIGHT_SMALL  , viewx-4*BORDERSIZE, LEFT, SMALL, ORANGE );
					if (i < 10)
						RenderString ( ":", 2*BORDERSIZE+138, 4*BORDERSIZE+ ( 2+i ) *FONTHEIGHT_SMALL  , 10, LEFT, SMALL, ORANGE );
				}
			}
		}
	}
	...
	for ( x = 0; x < 3; x++ )
	{
		for ( y = 0; y < 4; y++ )
		{
			...
			{	//keyboard
				RenderString ( szKeyBBoxKey[x+y*3],15 + x* ( KEYBOX_WIDTH+KEYBOX_SPACE ),VIEWY-INFOBOXY + KEYBOX_SPACE + FONTHEIGHT_SMALL + y* ( KEYBOX_HEIGHT+KEYBOX_SPACE ),40,LEFT, SMALL, WHITE );
				RenderString ( ( osd == 'G' ) ? szKeyBBoxInfoDe[x+y*3] : szKeyBBoxInfoEn[x+y*3],45 + x* ( KEYBOX_WIDTH+KEYBOX_SPACE ),VIEWY-INFOBOXY + KEYBOX_SPACE + FONTHEIGHT_SMALL + y* ( KEYBOX_HEIGHT+KEYBOX_SPACE ), KEYBOX_WIDTH - 40, RIGHT, SMALL, ORANGE );
			}
			else
			{	//Abc, abc
				RenderString ( szKeyBoxKey[x+y*3],15 + x* ( KEYBOX_WIDTH+KEYBOX_SPACE ),VIEWY-INFOBOXY + KEYBOX_SPACE + FONTHEIGHT_SMALL + y* ( KEYBOX_HEIGHT+KEYBOX_SPACE ),25,LEFT, SMALL, WHITE );
				RenderString ( szKeyBoxInfo[x+y*3],30 + x* ( KEYBOX_WIDTH+KEYBOX_SPACE ),VIEWY-INFOBOXY + KEYBOX_SPACE + FONTHEIGHT_SMALL + y* ( KEYBOX_HEIGHT+KEYBOX_SPACE ), KEYBOX_WIDTH - 25, RIGHT, SMALL, ORANGE );
			}
		}
	}
}
...
//	char szT9Code[11];
	...
	while ( 1 )
	{
		...
		int y = nEditType * 30;
		RenderCircle ( 3*(KEYBOX_WIDTH + KEYBOX_SPACE) + 35, VIEWY-INFOBOXY+10, 'D' );
		RenderCircle ( 3*(KEYBOX_WIDTH + KEYBOX_SPACE) + 35, VIEWY-INFOBOXY+40, 'N' );
		RenderCircle ( 3*(KEYBOX_WIDTH + KEYBOX_SPACE) + 35, VIEWY-INFOBOXY+70, 'O' );
		...
		{		// Absender
			...
			RenderString ( ( osd == 'G' ) ? "Texte" : "Letters", 3*(KEYBOX_WIDTH + KEYBOX_SPACE) + 60, VIEWY-INFOBOXY+25  , 100, CENTER, ( nEditType == 0 ) ? NORMAL : SMALL, ( nEditType == 0 ) ? ORANGE : WHITE );
			RenderString ( ( osd == 'G' ) ? "T9" : "T9", 3*(KEYBOX_WIDTH + KEYBOX_SPACE) + 60, VIEWY-INFOBOXY+55  , 100, CENTER, ( nEditType == 1 ) ? NORMAL : SMALL, ( nEditType == 1 ) ? ORANGE : WHITE );
		}
		else
		{
		...
		RenderBox ( 3*(KEYBOX_WIDTH + KEYBOX_SPACE) + 60, VIEWY-INFOBOXY+y+3  , 3*(KEYBOX_WIDTH + KEYBOX_SPACE) + 200, VIEWY-INFOBOXY+35+y, GRID, SKIN2 );
		...
		if ( nEditType == 0 )
		{
			...
			if ( pipe == NULL )
			{
				if ( !nTextFileIdx )
				{
					RenderString ( ( osd == 'G' ) ? "keine gelesene email" : "no read email", 2*BORDERSIZE, 2*BORDERSIZE+3*FONTHEIGHT_BIG, 500, LEFT, BIG, ORANGE );
				}
				else
				{
					RenderString ( ( osd == 'G' ) ? "kein Textblock" : "no Letter", 2*BORDERSIZE, 2*BORDERSIZE+3*FONTHEIGHT_BIG, 500, LEFT, BIG, ORANGE );
				}
				TextFileValid = 0;
			}
			else
			{
				...
				while ( fgets ( linebuffer, sizeof ( linebuffer ), pipe ) )
				{
					...
					RenderString ( linebuffer, 2*BORDERSIZE, 4*BORDERSIZE+ ( 2+nEditLine ) *FONTHEIGHT_SMALL, viewx-4*BORDERSIZE, LEFT, SMALL, WHITE );
		...
		else if ( nEditType == 2 )
		{
			...
			for ( i=0; i<MAXINFOLINES; i++ )
			{
				...
				if (nConfigPage == 11) //set different offset to ConfigPage
					xoff = 163;
				else if	(nConfigPage == 10)
					xoff = 168;
				else if (nConfigPage != -1)
					xoff = 100;
				...
				if ( nEditLine ==  i )
				{
					if ( szInfo[i][0] == '\0' )
					{
						RenderString ( "_", 2*BORDERSIZE+xoff, 4*BORDERSIZE+ ( 2+i ) *FONTHEIGHT_SMALL  , viewx-4*BORDERSIZE-xoff, LEFT, NORMAL, ORANGE );
					}
					else
					{
						int linelen, x;
						...
						if ( nEditPos )
						{
							...
							linepart[nEditPos] = '\0';
							RenderString ( linepart, 2*BORDERSIZE+xoff, 4*BORDERSIZE+ ( 2+i ) *FONTHEIGHT_SMALL  , viewx-4*BORDERSIZE-xoff, LEFT, NORMAL, WHITE );
						...
						linepart[1] = '\0';
						RenderString ( linepart, 2*BORDERSIZE+xoff+x, 4*BORDERSIZE+ ( 2+i ) *FONTHEIGHT_SMALL  , viewx-4*BORDERSIZE-xoff, LEFT, NORMAL, ORANGE );
						x += GetStringLen ( linepart );
						...
						if ( nEditPos != linelen )
						{
							...
							RenderString ( linepart, 2*BORDERSIZE+xoff+x, 4*BORDERSIZE+ ( 2+i ) *FONTHEIGHT_SMALL  , viewx-4*BORDERSIZE-xoff, LEFT, NORMAL, WHITE );
						}
					}
				}
				else
				{
					RenderString ( szInfo[i], 2*BORDERSIZE+xoff, 4*BORDERSIZE+ ( 2+i ) *FONTHEIGHT_SMALL  , viewx-4*BORDERSIZE-xoff, LEFT, SMALL, WHITE );
				}
			}
		}
		...
		*/
		...
		if ( nEditDirectStyle == 3 )
		{
			if ( ( rccode>=0x20 ) && ( rccode<0x100 ) )
			{
				...
				{
					if (nEditPos < (int)strlen(szInfo[nEditLine]))
					{
						nEditPos ++;
					}
					else
				...
				case RC_0:
					if ( nEditType == 2 )
					{
						...
						else
						{
							...
							for ( j = 0; j < (int)strlen(szKeyBoxInfo[iIndex]); j++ )
							{
								if ( szKeyBoxInfo[iIndex][j] == cAkt )
								{
									if ( ( j+1 ) < (int)strlen(szKeyBoxInfo[iIndex]))
									{
										j++;
									}
									else
...
void PaintMessageBox (int width, int height, int button)
{
	...
	RenderBox (mx-bwn, my-bhn   , mx+bwn, my-bhn+42, FILL, SKIN0);
	RenderBox (mx-bwn, my-bhn+42, mx+bwn, my+bhn   , FILL, SKIN1);
	RenderBox (mx-bwn, my-bhn   , mx+bwn, my+bhn   , GRID, SKIN2);
	RenderBox (mx-bwn, my-bhn+42, mx+bwn, my+bhn   , GRID, SKIN2);

	if (button == BUT_EXIT || button == BUT_OK)
	{
		// OK-Button background
		RenderBox (mx-35, my+bhn-42, mx+35, my+bhn-18, FILL, SKIN2);
		RenderString (button == 2 ? "OK" : "EXIT", mx-33, my+bhn-22, 66, CENTER, SMALL, WHITE);
	}
	else if (button == BUT_OKEXIT)
	{
		// OK + Exit
		RenderBox (mx-110, my+bhn-42, mx-40,  my+bhn-18, FILL, SKIN2);
		RenderBox (mx+40,  my+bhn-42, mx+110, my+bhn-18, FILL, SKIN2);
		RenderString ("OK",  mx-108,  my+bhn-22, 66, CENTER, SMALL, WHITE);
		RenderString ("EXIT", mx+42,  my+bhn-22, 66, CENTER, SMALL, WHITE);
	}
...
{
	char info[32];
	int bwb  = MBOX_WIDTH_BIG / 2;
	int maxw = MBOX_WIDTH_BIG - 4;
	
	
	// layout Paint Messagebox

	if (message != GETMAIL)
		PaintMessageBox (MBOX_WIDTH_BIG, MBOX_HEIGHT_NORMAL, BUT_OK);
	else
		PaintMessageBox (MBOX_WIDTH_BIG, MBOX_HEIGHT_NORMAL, 0);

	// message

	if ( message != INFO )
	{
		RenderString ( "TuxMail Statusinfo", mx-bwb+2, my-bh+35, maxw, CENTER, BIG, ORANGE );
	}
	...
	switch ( message )
	{
		case NODAEMON:
			...
			RenderString ( ( osd == 'G' ) ? "Daemon ist nicht geladen!" : "Daemon not running!", mx-bwb+2, my-bh+92, maxw, CENTER, BIG, WHITE );

			break;

		case STARTDONE:

			RenderString ( ( osd == 'G' ) ? "Abfrage wurde gestartet." : "Polling started.", mx-bwb+2, my-bh+92, maxw, CENTER, BIG, WHITE );

			break;

		case STARTFAIL:

			RenderString ( ( osd == 'G' ) ? "Start ist fehlgeschlagen!" : "Start failed!", mx-bwb+2, my-bh+92, maxw, CENTER, BIG, WHITE );

			break;

		case STOPDONE:

			RenderString ( ( osd == 'G' ) ? "Abfrage wurde gestoppt." : "Polling stopped.", mx-bwb+2, my-bh+92, maxw, CENTER, BIG, WHITE );

			break;

		case STOPFAIL:

			RenderString ( ( osd == 'G' ) ? "Stop ist fehlgeschlagen!" : "Stop failed!", mx-bwb+2, my-bh+92, maxw, CENTER, BIG, WHITE );

			break;

		case BOOTON:

			RenderString ( ( osd == 'G' ) ? "Autostart aktiviert." : "Autostart enabled.", mx-bwb+2, my-bh+92, maxw, CENTER, BIG, WHITE );

			break;

		case BOOTOFF:

			RenderString ( ( osd == 'G' ) ? "Autostart deaktiviert." : "Autostart disabled.", mx-bwb+2, my-bh+92, maxw, CENTER, BIG, WHITE );

			break;

		case ADD2SPAM:

			RenderString ( ( osd == 'G' ) ? "Spamliste wurde erweitert." : "Added to Spamlist.", mx-bwb+2, my-bh+92, maxw, CENTER, BIG, WHITE );

			break;

		case DELSPAM:

			RenderString ( ( osd == 'G' ) ? "von Spamliste entfernt." : "Removed from Spamlist.", mx-bwb+2, my-bh+92, maxw, CENTER, BIG, WHITE );

			break;

		case SPAMFAIL:

			RenderString ( ( osd == 'G' ) ? "Update fehlgeschlagen!" : "Update failed!", mx-bwb+2, my-bh+92, maxw, CENTER, BIG, WHITE );

			break;

		case GETMAIL:

			RenderString ( ( osd == 'G' ) ? "Mail wird gelesen." : "Reading Mail.", mx-bwb+2, my-bh+92, maxw, CENTER, BIG, WHITE );
			RenderString ( ( osd == 'G' ) ? "Moment bitte..." : "Please wait...", mx-bwb+2, my-bh+132, maxw, CENTER, BIG, WHITE );

			break;

		case GETMAILFAIL:

			RenderString ( ( osd == 'G' ) ? "Mail lesen fehlgeschlagen!" : "Reading Mail failed!", mx-bwb+2, my-bh+92, maxw, CENTER, BIG, WHITE );

			break;

		case SENDMAILDONE:

			RenderString ( ( osd == 'G' ) ? "Mail wurde gesendet." : "Mailing successful.", mx-bwb+2, my-bh+92, maxw, CENTER, BIG, WHITE );

			break;

		case SENDMAILFAIL:

			RenderString ( ( osd == 'G' ) ? "Mail nicht gesendet!" : "Mailing failed!", mx-bwb+2, my-bh+92, maxw, CENTER, BIG, WHITE );
		...
		case INFO:
			...
			RenderString ( info, mx-bwb+2, my-bh+35, maxw, CENTER, BIG, ORANGE );
			RenderString ( "(c) 2003-2005 Thomas \"LazyT\" Loewe", mx-bwb+2, my-bh+74, maxw, CENTER, SMALL, WHITE );
			RenderString ( "(c) 2005-2006 Robert \"robspr1\" Spreitzer", mx-bwb+2, my-bh+100, maxw, CENTER, SMALL, WHITE );
	}
...
/*	if ( lcd != -1 )
	...
	RenderBox ( 0, 0, viewx, viewy, FILL, SKIN0 );
	RenderBox ( 0, 42, viewx-26, viewy, FILL, SKIN1 );
	RenderBox ( 0, 0, viewx, viewy, GRID, SKIN2 );
	RenderBox ( 0, 42, viewx-26, viewy, GRID, SKIN2 );
	RenderBox ( viewx-27, 42, viewx, 69, GRID, SKIN2 );
	RenderBox ( viewx-27, viewy-27, viewx, viewy, GRID, SKIN2 );

	
	// status

	RenderString ( maildb[account].nr, 12, 34, 20, LEFT, BIG, ORANGE );
	RenderString ( maildb[account].time, 50, 34, 90, RIGHT, BIG, ORANGE );
	RenderString ( maildb[account].name, 150, 34, viewx-300, CENTER, BIG, ORANGE );
	RenderString ( maildb[account].status, viewx - 150, 34, 140, RIGHT, BIG, ORANGE );

	// scrollbar

	mailindex = ( mailindex/10 ) *10;

	RenderArrow (viewx - 20, 49, UP, WHITE);
	RenderArrow (viewx - 20, viewy - 20, DOWN, WHITE);

	scrollbar_len = (viewy - 103) / ( ( maildb[account].mails - 1 ) /10 + 1 );
	scrollbar_ofs = scrollbar_len*mailindex / 10;
	scrollbar_cor = (viewy - 103) - ( ( (viewy - 103)/scrollbar_len ) *scrollbar_len );
	RenderBox ( viewx - 22, 73 + scrollbar_ofs, viewx - 5, 73 + scrollbar_ofs + scrollbar_len + scrollbar_cor - 1, FILL, SKIN2 );
	...
	//	sim_key = 1;
		printf("_OUT_CHECKPIN RCCODE= %d\n",rccode);
		return;
	}
	else
	{
		// clear Messageboxtext
		RenderBox (mx-bw, my-bh, mx+bw, my+bh, FILL, SKIN1);
	}
	...
	{
		RenderBox ( 2, 44 + ( selectbar_mailindex%10 ) *46, viewx - 27, 44 + ( selectbar_mailindex%10 ) *46 + 44, FILL, SKIN2 );
	}
	...
	for ( loop = 0; loop < 10; loop++ )
	{
		RenderString ( ( char* ) &maildb[account].mailinfo[mailindex + loop].date, 2, sy, 76, RIGHT, SMALL, WHITE );
		RenderString ( ( char* ) &maildb[account].mailinfo[mailindex + loop].time, 2, sy + 22, 76, RIGHT, SMALL, WHITE );
		RenderString ( ( char* ) &maildb[account].mailinfo[mailindex + loop].from, 120, sy, viewx-146, LEFT, SMALL, WHITE );
		RenderString ( ( char* ) &maildb[account].mailinfo[mailindex + loop].subj, 120, sy + 22, viewx-146, LEFT, SMALL, WHITE );

		RenderCircle ( 90, sy - 4, maildb[account].mailinfo[mailindex + loop].type[0]);
...
int CheckPIN ( int Account )
{
	...
	if ( Account == -1 )
	{
		...
		printf("Config_pcode= %s, _ppincount= %d\n",pcode, *ppincount);
	}
	else
	{
		pcode = &maildb[Account].code[0];
		ppincount = &maildb[Account].pincount;
		printf("Acount_pcode= %s, _ppincount= %d\n",pcode, *ppincount);
	}
	...
	// account locked?RenderBox

	if ( *ppincount == 3 )
	{
		PaintMessageBox (MBOX_WIDTH_NORMAL, MBOX_HEIGHT_NORMAL, 0);

		RenderString ( ( osd == 'G' ) ? "Sicherheitsabfrage_1" : "Security Check", mx-bw+2, my-bh+35, maxw, CENTER, BIG, ORANGE );
		if ( Account == -1 ) //config locked
		{
			RenderString ( ( osd == 'G' ) ? "Konfiguration gesperrt!" : "config locked!", mx-bw+2, my-bh+92, maxw, CENTER, BIG, WHITE );
		}
		else	//account locked
		{
			RenderString ( ( osd == 'G' ) ? "Konto gesperrt!" : "Account locked!", mx-bw+2, my-bh+92, maxw, CENTER, BIG, WHITE );
			RenderString ( ( osd == 'G' ) ? "Anderes Konto mit 0-9" : "Another Account with 0-9", mx-bw+2, my-bh+132, maxw, CENTER, SMALL, WHITE );
		}
	...
	// layout
	PaintMessageBox (MBOX_WIDTH_NORMAL, MBOX_HEIGHT_NORMAL, BUT_EXIT);

	RenderString ( (osd == 'G') ? "Sicherheitsabfrage_2" : "Security Check", mx-bw+2, my-bh+35, maxw, CENTER, BIG, ORANGE );
	if (Account == -1)
		RenderString ( (osd == 'G') ? "Konfigurationszugriff:" : "Config access", mx-bw+2, my-bh+65, maxw, CENTER, SMALL, WHITE );
	else
		RenderString ( (osd == 'G') ? "Kontozugriff:" : "Account access", mx-bw+2, my-bh+65, maxw, CENTER, SMALL, WHITE );
	RenderString ( "PIN-Code :", mx-130, my-bh+100, 155, RIGHT, BIG, WHITE );
	RenderString ( "????", mx+50, my-bh+100, 100, LEFT, BIG, WHITE );
	...
	do
	{
		...
		switch ( rccode )
		{
			...
			case RC_2:
				printf("__switch RCCODE= %d, RC_2 = %d\n",rccode, RC_2);
				code[count] = '2';
				break;
		...
		if ( !skip )
		{
			RenderBox ( mx+35, my-bh+68, mx+bw-2, my-bh+103, FILL, SKIN1 );

			switch ( ++count )
			{
				case 1:
					RenderString ( "*???", mx+35, my-bh+100, 100, LEFT, BIG, WHITE );
					break;
	...
	{
		RenderBox ( mx-bw+2, my-bh+44, mx+bw-2, my+bh-2, FILL, SKIN1 );
		RenderString ( ( osd == 'G' ) ? "Zugriff verweigert!" : "Access denied!", mx-bw+2, my-bh+92, maxw, CENTER, BIG, WHITE );
		if ( Account != -1 )
		{
			RenderString ( ( osd == 'G' ) ? "Anderes Konto mit 0-9" : "Another Account with 0-9", mx-bw+2, my-bh+132, maxw, CENTER, SMALL, WHITE );
		}
...
int main ( void )
{
	...
	fb = rc = lcd = sx = ex = sy = ey = preset = -1;
#if 0
...
#endif
	read_neutrino_osd_conf ( &ex,&sx,&ey, &sy, &preset);
	...
	if (preset != 1 || ex - sx < 1050)
	{	viewx = ex - sx - 80;
		startx = sx + ((ex - sx - viewx) / 2);
	}
	else
	{
		viewx = ex - sx - 240;
...
*/
	pal = (skin == 1) ? 0 : (skin == 2) ? 1 : (skin == 3) ? 2 : 0;
